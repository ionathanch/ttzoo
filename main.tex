...
\begin{align*}
    ...
    &\nil: \List ~ A \\
    &\cons : (a: A) \to (l: \List ~ A) \to \List ~ A
\end{align*}
\begin{align*}
    &\const{lrec}: (A: \Type) \to (P: (l: \List ~ A) \to \Type) \to (pn: P ~ \nil) \\
    &\qquad\to (ps: (a: A) \to (l: \List ~ A) \to (pl: P ~ l) \to P ~ (\cons ~ a ~ l)) \\
...
%
\begin{align*}
    \data ~ &\Vector ~ (A: \Type): (n: \Nat) \to \Type ~ \where \\
    &\vnil: \Vector ~ A ~ \zero \\
    &\vcons: (n: \Nat) \to (a: A) \to (v: \Vector ~ A ~ n) \to \Vector ~ A ~ (\succ n)
\end{align*}
\begin{align*}
    &\const{vrec}: (A: \Type) \to (P: (n: \Nat) \to (v: \Vector ~ A ~ n) \to \Type) \to (pvn: P ~ \zero ~ \vnil) \\
    &\qquad\to (pvc: (n: \Nat) \to (a: A) \to (v: \Vector ~ A ~ n) \to (pv: P ~ n ~ v) \to P ~ (\succ ~ n) ~ (\vcons ~ n ~ a ~ v)) \\
...
Occasionally the type is written as $\W (x: A). B(x)$, just as function types may be written as $\Pi (x: A). B(x)$. 
