...
\begin{mathpar}
    ...
    \inferrule*[right=elim-W]{
        ...
        \Gamma \vdash w: \W ~ A ~ B
        \\\\
        \Gamma \vdash p: (a: A) \to (b: B ~ a \to W ~ A ~ B) \to (pw: (x: B ~ a) \to P ~ (b ~ x)) \to P ~ (\sup ~ a ~ b)
    }{
    ...
    \inferrule*[right=comp-W]{
        ...
        \Gamma \vdash \const{wrec} ~ A ~ B ~ P ~ p ~ (\sup ~ a ~ b) \equiv p ~ a ~ b ~ (\lambda x: B ~ a. ~ \const{wrec} ~ A ~ B ~ P ~ p ~ (b ~ x)) : P ~ (\sup ~ a ~ b)
    }
\end{mathpar}
