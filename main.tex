...
\newcommand{\ite}[3]{\const{if} ~ #1 ~ \const{then} ~ #2 ~ \const{else} ~ #3}
\newcommand{\pair}[5]{(#1, #2) ~ \const{as} ~ (#3: #4 \times #5)}
\renewcommand{\fst}{\const{fst}}
\renewcommand{\snd}{\const{snd}}
...
\begin{mathpar}
    \inferrule*[Right=match]{
        ...
        \\\\
        \Gamma \Delta_i \vdash d_i[\Vec{z}_i \mapsto \Delta_i] : P ~ \Vec{e}[\Delta_P \mapsto \Vec{p}] ~ (c_i ~ \Vec{p} ~ \Delta_i)
    }{
        \Gamma \vdash \match ~ x.\Vec{y}.P ~ t ~ \with ~ \langle c_i ~ \Vec{z}_i \Rightarrow d_i \rangle_i : P[x \mapsto t][\Vec{y} \mapsto \Vec{e}]
    }
\end{mathpar}

This results in function application in the computation rule for case expressions and substitution in the computation rule for match expressions. Since they don't rely on any of the types and clearly compute in one direction, we can instead write them as untyped reduction rules, sometimes referred to as \emph{$\iota$-reduction}.
%
\begin{align*}
    \case ~ P ~ (c_j ~ \Vec{p} ~ \Vec{a}) ~ \of ~ \langle c_i \Rightarrow d_i \rangle_i &\rhd_\iota d_j ~ \Vec{a} \\
    \match ~ x.\Vec{y}.P ~ (c_j ~ \Vec{p} ~ \Vec{a}) ~ \with ~ \langle c_i ~ \Vec{z}_i \Rightarrow d_i \rangle_i &\rhd_\iota d_j[\Vec{z}_j \mapsto \Vec{a}]
\end{align*}

\subsubsection{Uniqueness Rules}\label{uniqueness}

There is also a uniqueness rule for case and match expressions, but it renders the type theory undecidable (in particular, it allows for equality reflection, as seen in \autoref{ext-eq}). We state the rules here for reference.
%
\begin{mathpar}
    \inferrule*[right=case-uniq]{
        \dots
        \and
        \Gamma \vdash t: I ~ \Vec{p} ~ \Vec{e}
        \and
        \Gamma (x: I ~ \Vec{p} ~ \Vec{e}) \vdash d: P ~ \Vec{e} ~ x
    }{
        \Gamma \vdash \case ~ P ~ t ~ \of ~ \langle c_i \Rightarrow \lambda \Delta_i. ~ d[x \mapsto c_i ~ \Vec{p} ~ \Delta_i] \rangle_i \equiv d[x \mapsto t] : P ~ \Vec{e} ~ t
    }
    
    \inferrule*[right=match-uniq]{
        \dots
        \and
        \Gamma \vdash t: I ~ \Vec{p} ~ \Vec{e}
        \and
        \Gamma (x: I ~ \Vec{p} ~ \Vec{e}) \vdash d: P[\Vec{y} \mapsto \Vec{e}]
    }{
        \Gamma \vdash \match ~ x.\Vec{y}.P ~ t ~ \of \langle c_i ~ \Vec{z}_i \Rightarrow d[x \mapsto c_i ~ \Vec{p} ~ \Vec{z}] \rangle_i \equiv d[x \mapsto t] : P[x \mapsto t][\Vec{y} \mapsto \Vec{e}]
    }
\end{mathpar}

This uniqueness rule can instead be split into a number of simpler rules \citep{eta}, some of which are called $\eta$- or $\gamma$-reduction rules. We present a few of them here in a manner that suggests at a decidable algorithm for checking equality by using the reflexive, transitive, congruent closure $\rhd^*$ of our usual reduction rules. The first one says that if all that the case or match expression does is to rebuild the destructed target, then it should be equal to the target itself.
%
\begin{mathpar}
    \inferrule*[right=case-$\eta_1$]{
        \Gamma \vdash e_1 \rhd^* \case ~ P ~ e_1' ~ \of ~ \langle c_i \Rightarrow d_i \rangle_i
        \\\\
        \Gamma \vdash e_2 \rhd^* e_2'
        \and
        \Gamma \vdash d_i \equiv c_i ~ \Vec{p} : \Delta_i \to I ~ \Vec{p} ~ \Vec{e}
        \and
        \Gamma \vdash e_1' \equiv e_2' : I ~ \Vec{p} ~ \Vec{e}
    }{
        \Gamma \vdash e_1 \equiv e_2 : I ~ \Vec{p} ~ \Vec{e}
    }
    
    \inferrule*[right=match-$\eta_1$]{
        \Gamma \vdash e_1 \rhd^* \match ~ x.\Vec{y}.P ~ e_1' ~ \with ~ \langle c_i ~ \Vec{z}_i \Rightarrow d_i \rangle_i
        \\\\
        \Gamma \vdash e_2 \rhd^* e_2'
        \and
        \Gamma \Delta_i \vdash d_i[\Vec{z}_i \mapsto \Delta_i] \equiv c_i ~ \Vec{p} ~ \Delta_i : I ~ \Vec{p} ~ \Vec{e}
        \and
        \Gamma \vdash e_1' \equiv e_2' : I ~ \Vec{p} ~ \Vec{e}
    }{
        \Gamma \vdash e_1 \equiv e_2 : I ~ \Vec{p} ~ \Vec{e}
...
The second one is also known as a \emph{redundant guard} or \emph{indifference}: if all of the branches are equal and independent of the constructor's arguments, then the entire case or match expression should be equal to that branch.
%
\begin{mathpar}
    \inferrule*[right=case-$\eta_2$]{
        \Gamma \vdash e_1 \rhd^* \case ~ P ~ t ~ \of ~ \langle c_i \Rightarrow d_i \rangle_i
        \\\\
        \Gamma \vdash e_2 \rhd^* e_2'
        \and
        \Gamma \vdash d_i \equiv \lambda \Delta_i. ~ e_2' : \Delta_i \to P ~ \Vec{e} ~ t
    }{
        \Gamma \vdash e_1 \equiv e_2: P ~ \Vec{e} ~ t
    }
    
    \inferrule*[right=match-$\eta_2$]{
        \Gamma \vdash e_1 \rhd^* \match ~ x.\Vec{y}.P ~ t ~ \with ~ \langle c_i ~ \Vec{z}_i \Rightarrow d_i \rangle_i
        \\\\
        \Gamma \vdash e_2 \rhd^* e_2'
        \and
        \Gamma \vdash d_i \equiv e_2' : P[x \mapsto t][\Vec{y} \mapsto \Vec{e}]
    }{
        \Gamma \vdash e_1 \equiv e_2: P[x \mapsto t][\Vec{y} \mapsto \Vec{e}]
    }
\end{mathpar}

\iffalse
They are called $\eta$ or $\gamma$ laws; we stick to calling all of them $\eta$. The first could be considered as the traditional $\eta$-reduction rules: if we rebuild the inductive datum from the pieces obtained from decomposition, then we should simply return the original target.
%
\begin{align*}
    \case ~ P ~ t ~ \of ~ \langle c_i \Rightarrow \lambda \Delta_i. ~ c_i ~ \Delta_i \rangle_i &\rhd_{\eta} t \\
    \match ~ x.\Vec{y}.P ~ t ~ \with ~ \langle c_i ~ \Vec{z}_i \Rightarrow c_i ~ \Vec{z} \rangle_i &\rhd_{\eta} t
\end{align*}

Sometimes this rule isn't strong enough, as we will see in \autoref{wtypes}. We could add what are also known as \emph{filling-up rules} \citep{well-ordering}.
%
\begin{align*}
...
\begin{mathpar}
    \inferrule*[Right=comp]{
        ...
        \Gamma \vdash \const{elim}_I ~ \Vec{p} ~ P ~ \langle P_i \rangle_i ~ \Vec{e} ~ (c_j ~ \Vec{a}) \rhd_\mu P_j ~ \Vec{a} ~ \langle \lambda \Delta_{k}. ~ \const{elim}_I ~ \Vec{p} ~ P ~ \langle P_i \rangle_i ~ \Vec{e}_k ~ (a_k ~ \Delta_k) \rangle_{k \in j\mathbb{J}} : P ~ \Vec{e} ~ (c_j ~ \Vec{a})
    }
\end{mathpar}
\iffalse
\begin{align*}
...
% TODO: Update to bring in line with the methodology for deep induction
% TODO: Add an example for truly nested types (e.g. bushes)
...
\end{align*}
\begin{mathpar}
    \inferrule*[right=$\top$-uniq]{
        \Gamma \vdash e_1 \rhd^* \star
        \and
        \Gamma \vdash e_2 \rhd^* x
        \and
        (x: \top) \in \Gamma
    }{
        \Gamma \vdash e_1 \equiv e_2 : \top
    }
\end{mathpar}
...
\begin{mathpar}
    ...
    \inferrule*[right=comp-W]{
        ...
        \Gamma \vdash \const{wrec} ~ A ~ B ~ P ~ p ~ (\sup ~ A ~ B ~ a ~ b) \rhd_\mu p ~ a ~ b ~ (\lambda x: B ~ a. ~ \const{wrec} ~ A ~ B ~ P ~ p ~ (b ~ x)) : P ~ (\sup ~ A ~ B ~ a ~ b)
    }
\end{mathpar}

\subsection{Recovering Induction Principles?}

Being able to define other inductive types using W types, we should expect to also be able to define their respective eliminators. Taking $\Nat$ as an example, we attempt to define $\const{nrec}$ using the eliminator for W types, $\const{wrec}$.

\begin{align*}
    &\const{nrec} : (P: \Nat \to \Type) \to (pz: P ~ \zero) \\
    &\qquad\to (ps: (n: \Nat) \to P ~ n \to P ~ (\succ ~ n)) \\
    &\qquad\to (n: \Nat) \to P ~ n \\
    &\const{nrec} ~ P ~ pz ~ ps \coloneqq \const{wrec} ~ \Bool ~ (\lambda b: \Bool. ~ \case ~ b ~ \of ~ \langle \false \Rightarrow \bot, \true \Rightarrow \top \rangle) ~ P \\
    &\hspace{9.5em} (\lambda a: \Bool. ~ \case ~ a ~ \of \\
    &\hspace{15.5em} \false \Rightarrow (\lambda b: \bot \to \Nat. ~ (\lambda pw: (x: \bot) \to P ~ (b ~ x). ~ \hole)) \\
    &\hspace{15.5em} \true \Rightarrow (\lambda b: \top \to \Nat. ~ (\lambda pw: (x: \top) \to P ~ (b ~ x). ~ \hole)))
\end{align*}
...
\begin{mathpar}
    \inferrule*[right=form-W]{
        ...
        \Gamma \vdash \W ~ Z ~ A ~ B: \Type
    }
    
    \inferrule*[right=intro-W-zub]{
        \dots
        \and
        \Gamma \vdash z: Z
    }{
        \Gamma \vdash \zub ~ Z ~ A ~ B ~ z: \W ~ Z ~ A ~ B
    }
    
    \inferrule*[right=intro-W-sup]{
        \dots
        \and
        \Gamma \vdash a: A
        \and
        \Gamma (a: A) \vdash b: B ~ a \to \W ~ Z ~ A ~ B
    }{
        \Gamma \vdash \sup ~ Z ~ A ~ B ~ a ~ b: \W ~ Z ~ A ~ B
    }
    
    \inferrule*[right=elim-W]{
        \dots
        \and
        \Gamma \vdash P: \W ~ Z ~ A ~ B \to \Type
        \and
        \Gamma \vdash w: \W ~ Z ~ A ~ B
        \and
        \Gamma \vdash pz: (z: Z) \to P ~ (\zub ~ Z ~ A ~ B ~ z)
        \\\\
        \Gamma \vdash ps: (a: A) \to (b: B ~ a \to \W ~ Z ~ A ~ B) \to (pw: (x: B ~ a) \to P ~ (b ~ x)) \to P ~ (\sup ~ A ~ B ~ Z ~ a ~ b)
    }{
        \Gamma \vdash \const{wrec} ~ Z ~ A ~ B ~ P ~ pz ~ ps ~ w: P ~ w
    }
\end{mathpar}
\begin{align*}
    \const{wrec} ~ Z ~ A ~ B ~ P ~ pz ~ ps ~ (\zub ~ Z ~ A ~ B ~ z) &\rhd_{\mu} pz ~ z \\
    \const{wrec} ~ Z ~ A ~ B ~ P ~ pz ~ ps ~ (\sup ~ Z ~ A ~ B ~ a ~ b) &\rhd_{\mu} ps ~ a ~ b ~ (\lambda x: B ~ a. ~ \const{wrec} ~ Z ~ A ~ B ~ P ~ pz ~ ps ~ (b ~ x))
\end{align*}

The inhabitants of $Z$ select the base case that we need, just as those of $A$ select the step cases. To verify that this works, we again define the naturals using our new W type (omitting once more the type arguments $Z, A, B$).
%
\begin{align*}
    \Nat &\coloneqq \W ~ \top ~ \top ~ (\lambda x: \top. ~ \top) \\
    \zero &\coloneqq \zub ~ \star \\
    \succ ~ n &\coloneqq \sup \star (\lambda x: \top. ~ n)
\end{align*}

Now we are able to define the eliminator for naturals as expected.

\begin{align*}
    &\const{nrec} : (P: \Nat \to \Type) \to (pz: P ~ \zero) \\
    &\qquad\to (ps: (n: \Nat) \to P ~ n \to P ~ (\succ ~ n)) \\
    &\qquad\to (n: \Nat) \to P ~ n \\
    &\const{nrec} ~ P ~ pz ~ ps \coloneqq \const{wrec} ~ \top ~ \top ~ (\lambda x: \top. ~ \top) ~ P ~ (\lambda x: \top. ~ pz) \\
    &\hspace{9.5em} (\lambda x: \top. ~ \lambda b: \top \to \Nat. ~ \lambda pw: (x: \bot) \to P ~ (b ~ x). ~ ps ~ (b ~ \star) ~ (pw ~ \star))
\end{align*}
...
\begin{align*}
    ...
    &\kelim ~ A ~ a ~ P ~ d ~ p \coloneqq \subst ~ (a =_A a) ~ P ~ \refl_a ~ p ~ (\rip ~ A ~ a ~ a ~ p) ~ d
\end{align*}
...
\begin{mathpar}
    \inferrule*[right=$\equiv$]{
        \Gamma \vdash e_1 \rhd^* e
        \and
        \Gamma \vdash e_2 \rhd^* e
    }{
        \Gamma \vdash e_1 \equiv e_2
    }
    \\
    \inferrule*[right=$\equiv$-Lam-$\eta_1$]{
        \Gamma \vdash e_1 \rhd^* (\lambda x: A. ~ e)
        \and
        \Gamma \vdash e_2 \rhd^* e_2'
        \\\\
    ...
    \inferrule*[right=$\equiv$-Lam-$\eta_2$]{
        \Gamma \vdash e_1 \rhd^* e_1'
        \and
        \Gamma \vdash e_2 \rhd^* (\lambda x: A. ~ e)
        \\\\
    ...
    \\
    \inferrule*[right=$\equiv$-$\top$-$\eta_1$]{
        \Gamma \vdash e_1 \rhd^* \star
        \and
        \Gamma \vdash e_2 \rhd^* x
        \\\\
    ...
    \inferrule*[right=$\equiv$-$\top$-$\eta_2$]{
        \Gamma \vdash e_1 \rhd^* x
        \and
        \Gamma \vdash e_2 \rhd^* \star
        \\\\
        (x: \top) \in \Gamma
    }{
        \Gamma \vdash e_1 \equiv e_2
    ...
    \\
    \inferrule*[right=$\equiv$-If-$\eta_1$]{
        \Gamma \vdash e_1 \rhd^* \ite{e}{e_1' ~ \true}{e_1' ~ \false}
        \and
        \Gamma \vdash e_2 \rhd^* e_2'
        \and
        \Gamma \vdash e_1' ~ e \equiv e_2'
    }{
        \Gamma \vdash e_1 \equiv e_2
    }
    
    \inferrule*[right=$\equiv$-If-$\eta_2$]{
        \Gamma \vdash e_1 \rhd^* e_1'
        \and
        \Gamma \vdash e_2 \rhd^* \ite{e}{e_2' ~ \true}{e_2' ~ \false}
        \and
