...
Finally, we can give a kind of uniqueness rule for functions involving case or match expressions; as untyped reduction rules, we'll call it \emph{$\eta$-contraction}.
%
\begin{align*}
    \case ~ P ~ t ~ \of ~ \langle c_i \Rightarrow \lambda \Delta_i. ~ f ~ (c_i ~ \Delta_i) \rangle_i &\rhd_\eta f ~ t \\
    \match ~ x.\vec{y}.P ~ t ~ \with ~ \langle c_i ~ \vec{z} \Rightarrow f ~ (c_i ~ \vec{z}) \rangle_i &\rhd_\eta f ~ t
\end{align*}
